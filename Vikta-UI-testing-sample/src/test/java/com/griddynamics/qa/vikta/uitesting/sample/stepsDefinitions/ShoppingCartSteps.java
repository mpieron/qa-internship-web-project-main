...
public class ShoppingCartSteps extends BaseSteps {
    ...
    public void verifyPurchase(String totalPrice){
        ...
        assertThat(shoppingCartPage().getSpPurchaseTotal())
                .as("Purchase total should be %s but is %s", totalPrice, shoppingCartPage().getSpPurchaseTotal())
                .contains(totalPrice);

        assertThat(shoppingCartPage().getPurchaseDeliveredTo())
                .as("Delivery should be to %s, but was to %s",
                        shoppingCartPage().getSlctAddress().split("...")[1], shoppingCartPage().getPurchaseDeliveredTo())
        ...
        assertThat(shoppingCartPage().getPurchasePaidBy())
                .as("Payment should be by %s, but was by %s",
                        shoppingCartPage().getSlctPayment().split("...")[1], shoppingCartPage().getPurchasePaidBy())
    ...
    ShoppingCartPage shoppingCartPage(){ return getPage(ShoppingCartPage.class); }

    ImageDetailsPage imageDetailsPage() { return  getPage(ImageDetailsPage.class); }
