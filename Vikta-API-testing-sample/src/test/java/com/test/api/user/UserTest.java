...
public class UserTest extends BaseTest {
    ...
    public void canListAllUsers(){
        ...
        assertThat(usersList).as("Fields shouldn't be null").allSatisfy(user -> assertThat(user)
                        .hasNoNullFieldsOrProperties());

        assertThat(usersList).as("Fields shouldn't be empty").allSatisfy(user -> assertThat(user)
                .usingRecursiveComparison().isNotEqualTo(""));
    }
    ...
    public void canGetUserByLoginName(){
        ...
        UserDTO user = usersList.get(0);

        assertThat(usersList.size()).as("There should be no users with same login")
                .isEqualTo(1);

        assertThat(user.getLoginName())
                .as(String.format("Should be user with login %s, but was with %s", userLogin, user.getLoginName()))
                .isEqualTo(userLogin);
    }
    ...
    private static Stream<Arguments> goodParametersForPost() {
        String words64 = RandomStringUtils.randomAlphabetic(64);
        String words63 = RandomStringUtils.randomAlphabetic(63);
        String words3 = RandomStringUtils.randomAlphabetic(59);
        // this is max that I can apply, 319 length string, more won't pass
        String maxEmail = String.format("%s@%s.%s.%s.%s.com", words64, words63, words63, words63, words3);
        ...
        return Stream.of(
                ...
                Arguments.of("jan","password123",maxEmail , "Jan", "Janusz", "Kowalski", "avatar", new int[]{}, new int[]{}, "Case with max email length that pass"),
    ...
    private static Stream<Arguments> wrongParametersForPost() {
        String words64 = RandomStringUtils.randomAlphabetic(64);
        String words63 = RandomStringUtils.randomAlphabetic(63);
        String maxEmail = String.format("%s@%s.%s.%s.%s.%s.com", words64, words63, words63, words63, words63, words63);
        ...
        return Stream.of(
                ...
                Arguments.of("jan","password123", String.format("%s.%s", "j", maxEmail), "Jan", "Janusz", "Kowalski", "avatar", new int[]{}, new int[]{}, "Case with too long email"),
                Arguments.of("jan","password123", String.format("%s.%s", words63, maxEmail), "Jan", "Janusz", "Kowalski", "avatar", new int[]{}, new int[]{}, "Case with too much long email"),
